...
type ParsedArgs = Record<string, string | boolean | string[] | undefined> & {
  _: string[];
};
...
async function main() {
  ...
  if (command === "help") printUsageAndExit();
...
async function initCommand(args: ParsedArgs) {
  ...
  if (secret) env.push(`BASE_IDP_CLIENT_SECRET=${secret}`);
  ...
  if (boolArg(args, "post", false)) {
    ...
    if (!adminToken) printUsageAndExit("--post requires --admin-token");
...
async function testCommand(args: ParsedArgs) {
  const issuer = stringArg(args, "issuer") || process.env.BASE_IDP_ISSUER || "https://authlayer.squareexp.com";
  const key = stringArg(args, "key") || process.env.BASE_IDP_KEY;
...
async function tokenCommand(args: ParsedArgs) {
  const token = stringArg(args, "token") || args._[0];
  if (!token) printUsageAndExit("Missing token argument");
...
function stringArg(args: ParsedArgs, key: string): string {
  const value = args[key];
  return typeof value === "string" ? value : "";
}

function boolArg(args: ParsedArgs, key: string, fallback: boolean): boolean {
  const value = args[key];
